...
import { PageHeader, SearchBar, Table, Pagination, Modal, Loading, EmptyState, toast } from '@/components/base';
...
function OrderDetail({ order, onClose, onStatusChange }: OrderDetailProps) {
  ...
  const updateStatus = async (newStatus: string) => {
    ...
    try {
      ...
      toast((e as Error).message || 'Lỗi khi cập nhật trạng thái', 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
          ...
          <p className="font-medium text-gray-900 dark:text-white">{order.customerName || 'Khách lẻ'}</p>
        </div>
        <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
      ...
      <div className="bg-primary-50 dark:bg-primary-900/20 rounded-xl p-4 space-y-2">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className="text-gray-900 dark:text-white">{order.paymentMethod === 'cash' ? 'Tiền mặt' : 'Chuyển khoản'}</span>
        </div>
      </div>
      ...
      <div className="flex justify-end">
        <button className="btn btn-secondary text-sm px-4 py-2" onClick={onClose}>Đóng</button>
      </div>
    </div>
  );
}
...
export default function OrdersTable() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await orderService.getAll({ search, status: statusFilter, page, limit: 10 });
      setOrders(res.data);
      setTotalPages(res.totalPages || 1);
      setTotal(res.total || 0);
    } catch (e: unknown) {
      toast((e as Error).message || 'Lỗi khi tải đơn hàng', 'error');
    } finally {
      setLoading(false);
    }
  ...
  const viewOrder = async (order: Order) => {
    try {
      ...
      toast((e as Error).message || 'Lỗi khi tải chi tiết đơn hàng', 'error');
    }
  };
  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumb={['Đơn hàng', 'Danh sách đơn hàng']}
        actions={
          ...
          <>
            <Table headers={['Mã đơn', 'Khách hàng', 'Nhân viên', 'Ngày tạo', 'Tổng tiền', 'Trạng thái', 'Chi tiết']}>
              {orders.length === 0 ? (
                <tr>
                  <td colSpan={7}>
                    <EmptyState message="Không có đơn hàng nào" icon={ShoppingCart} />
                  </td>
                </tr>
              ) : (
                orders.map(order => (
                  <tr key={order.id} className="table-row">
                    ...
                    <td className="px-4 py-3 text-right text-sm text-gray-900 dark:text-white">{order.customerName || 'Khách lẻ'}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        ...
                        className="p-1.5 rounded-lg hover:bg-primary-50 dark:hover:bg-primary-900/20 text-primary-600 cursor-pointer"
                      >
                        <Eye size={14} />
                      </button>
                    </td>

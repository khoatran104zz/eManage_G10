...
import { PageHeader, SearchBar, Table, Loading, EmptyState } from '@/components/base';
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="card p-4">
          ...
          <p className="text-2xl font-bold mt-1 text-gray-900 dark:text-white">{filtered.length}</p>
        </div>
        <div className="card p-4">
        ...
        {loading ? (
          ...
          <Table headers={['Sản phẩm', 'SKU', 'Danh mục', 'Giá nhập', 'Giá bán', 'Tồn kho', 'Giá trị']}>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7}>
                  <EmptyState message="Không có sản phẩm" icon={Warehouse} />
                </td>
              </tr>
            ) : (
              filtered.map(p => (
                <tr key={p.id} className="table-row">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 shrink-0 flex items-center justify-center">
                        {p.image ? (
                  ...
                  <td className="px-4 py-3 text-right text-sm text-gray-900 dark:text-white">{formatCurrency(p.costPrice)}</td>
                  <td className="px-4 py-3 text-right text-sm text-gray-900 dark:text-white">{formatCurrency(p.salePrice)}</td>

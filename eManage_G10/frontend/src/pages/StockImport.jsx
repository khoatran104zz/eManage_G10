// pages/StockImport.jsx - Nhập kho
...
export default function StockImport() {
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      const res = await stockApi.import(form);
      toast(`Nhập kho thành công! Tồn kho mới: ${res.newStock}`);
      setForm({ productId: '', quantity: '', supplierId: '', note: '' });
      const h = await stockApi.getHistory('import');
      setHistory(h);
      const p = await productsApi.getAll({ limit: 100, page: 1 });
      setProducts(p.data);
  ...
  return (
    <div>
      <PageHeader title="Nhập kho" subtitle="Ghi nhận hàng hóa nhập vào kho" />
        ...
        <div className="card p-5">
          ...
          <FormField label="Ghi chú">
            <textarea className="input resize-none h-20" value={form.note}
              onChange={e => set('note', e.target.value)} placeholder="Ghi chú nhập kho..." />
          </FormField>
          <button className="btn btn-primary w-full justify-center" onClick={submit} disabled={saving}>
            <CheckCircle size={16} />

// pages/POS.jsx - Màn hình bán hàng POS
import { useState, useEffect, useCallback } from 'react';
import { Search, Plus, Minus, Trash2, ShoppingCart, User, CreditCard, Printer, CheckCircle, Package } from 'lucide-react';
...
function InvoiceModal({ order, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 font-mono text-sm">
...
export default function POS() {
  ...
  const loadProducts = useCallback(async () => {
    const res = await productsApi.getAll({ search, limit: 20, page: 1 });
    setProducts(res.data);
  }, [search]);

  const loadCustomers = useCallback(async () => {
    const data = await customersApi.getAll(customerSearch);
    setCustomers(data);
  }, [customerSearch]);

  useEffect(() => { loadProducts(); }, [loadProducts]);
  useEffect(() => { if (showCustomers) loadCustomers(); }, [loadCustomers, showCustomers]);
  ...
  const addToCart = (product) => {
    ...
    setCart(prev => {
      ...
      if (existing) {
        if (existing.quantity >= product.stock) return toast('Không đủ tồn kho', 'error'), prev;
        return prev.map(i => i.productId === product.id ? { ...i, quantity: i.quantity + 1 } : i);
      }
      return [...prev, {
        productId: product.id, productName: product.name,
        price: product.salePrice, quantity: 1, maxStock: product.stock
      }];
    });
  };
  ...
  const updateQty = (productId, delta) => {
    setCart(prev => prev
      .map(i => i.productId === productId ? { ...i, quantity: Math.max(1, Math.min(i.quantity + delta, i.maxStock)) } : i)
    );
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      const order = await ordersApi.create({
        ...
        employeeId: 'emp2',
        items: cart, discount: Number(discount),
        paymentMethod, note: ''
      });
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 content-start">
          {products.map(p => (
            ...
            >
              <div className="w-full aspect-square rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 mb-2">
                {p.image
      ...
      <div className="w-80 flex flex-col gap-3 shrink-0">
        ...
        <div className="card p-3 relative">
          ...
          {selectedCustomer ? (
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setSelectedCustomer(null)} className="text-xs text-red-500 hover:underline">Xóa</button>
            </div>
          ) : (
            <div>
              <input
              ...
              {showCustomers && filteredCustomers.length > 0 && (
                <div className="absolute top-full left-0 right-0 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg z-10 max-h-40 overflow-y-auto mt-1">
            ...
            {cart.length === 0 ? (
              ...
              <div key={item.productId} className="px-3 py-2">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button onClick={() => removeFromCart(item.productId)} className="text-red-400 hover:text-red-600 shrink-0">
                    <Trash2 size={12} />
                  </button>
                </div>
        ...
        <div className="card p-3 space-y-3">
          ...
          <div>
            ...
            <input
              type="number" className="input text-sm" min="0"
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[['cash', 'Tiền mặt'], ['transfer', 'Chuyển khoản']].map(([v, l]) => (
                <button key={v}
                  onClick={() => setPaymentMethod(v)}
                  className={`text-xs py-2 rounded-lg border font-medium transition-all ${paymentMethod === v ? 'bg-primary-500 text-white border-primary-500' : 'border-gray-300 dark:border-gray-700 hover:border-primary-400'}`}>
                  {l}
                </button>
              ))}
            </div>

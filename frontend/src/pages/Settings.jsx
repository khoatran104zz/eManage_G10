// pages/Settings.jsx - Cài đặt hệ thống
import { useState, useEffect } from 'react';
import { settingsApi } from '../services/api';
import { useTheme } from '../context/ThemeContext';
import { PageHeader, FormField, Loading, toast } from '../components/shared';
...
export default function Settings() {
  ...
  return (
    <div className="max-w-2xl">
      <PageHeader title="Cài đặt" subtitle="Thông tin cửa hàng và tùy chỉnh hệ thống" />
      ...
      <div className="card p-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={toggle}
            className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors duration-300 ${dark ? 'bg-primary-500' : 'bg-gray-300'}`}
          >
            <span className={`inline-flex items-center justify-center w-5 h-5 bg-white rounded-full shadow transform transition-transform duration-300 ${dark ? 'translate-x-6' : 'translate-x-1'}`}>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <div
            onClick={() => dark && toggle()}
            className={`cursor-pointer rounded-xl border-2 p-4 transition-all ${!dark ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/10' : 'border-gray-200 dark:border-gray-700'}`}
          >
          ...
          <div
            ...
            className={`cursor-pointer rounded-xl border-2 p-4 transition-all ${dark ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/10' : 'border-gray-200 dark:border-gray-700'}`}
          >
            <div className="w-full h-12 bg-gray-900 rounded-lg border border-gray-700 mb-2 flex items-center px-2 gap-2">
